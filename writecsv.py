...
covidDS = r"D:\data\covid\cases\covid19_.csv"
...
if __name__ == '__main__':   
    ...
    data=ans.replace('\r','').replace('/','_')
    with open(fullDS,'w') as fileFull:
        fileFull.write(data) 
    fileFull.close() 
    df=pd.read_csv(fullDS,header=0)  
    for state in states.keys():
        print (df[df.Province_State==state])
        stateDS = os.path.join(r"D:\data\covid\cases",state+"_covid19_"+tdate+".csv")
        df[df.Province_State==state].to_csv(stateDS,index=None,header = df.columns,)    
