...
if __name__ == '__main__':   
    ...
    for k,v in states.item():
        ...
        for rows in range(2,len(test),7):
            sdfTMax=0
            for col in range(0,len(test.columns)):
                if test.iloc[rows][col] > sdfTMax:
                    sdfTMax=test.iloc[rows][col]
            test[:rows].plot(figsize=(16,12),ylim=(0,sdfTMax),xlim=(0,len(test[:rows])))
            plt.legend( loc='upper left', borderaxespad=0., ncol=5) 
