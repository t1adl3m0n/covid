...
def CountFrequency(my_list):  
    ...
    for k, v in freq.items(): 
        if k-lastValue > v:
            freq[k]=math.ceil((k-lastValue)/v)
            lastValue =k
#        print ("% d : % d"%(k, v)) 
    return freq
newDF=[]
newCdf=pd.DataFrame(columns=['FIPS','Admin2','Province_State','DATE','COUNT','COUNTDIFF'])# create new cases per day dataframe 
...
#        stateDF=sDF[sDF.FIPS.notnull()]
#        with open(newDaystatefile,'w') as nDstatefile: #open new csv file to write data to
#            nDstatefile.write("UID,GEONUM,COUNTY,STATE,DATE,COUNT,COUNTDIFF\n")# write headers
...
#                countyInfo=pd.DataFrame.from_records(cdf,columns=['FIPS','Admin2','Province_State'])
#                for col in range(11,len(cdf.columns)):
#                    newRow = countyInfo.to_csv().split('\n')[1].replace('\r','')+','+ str(dt.datetime.strptime(cdf.columns[col],'%m_%d_%y'))[:10]+','+str(cdf.values[0][col])+','+str(freqL[cdf.values[0][col]])
...
#        nDstatefile.close()
        for st in capstoneStates:
            ...
            sDF=ndf[ndf['STATE']==st] #Create new data frame with state specific data
