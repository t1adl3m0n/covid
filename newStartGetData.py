...
if __name__ == '__main__':    
    try:
        ...
        for row in range(0,len(sDF)):
            orgV=pd.DataFrame(columns=['STATE','COUNTY','GEONUM','DATE','COUNT','PERDAY'])
            numF=pd.DataFrame()
            gDF=sDF[row:row+1]
            orgV['DATE']=gDF.T[11:].index.values
            orgV['DATE']=orgV['DATE'].str.replace("_","-")
            orgV['COUNT']=gDF.T[11:].values.astype('int')
            orgV['STATE']=gDF.Province_State.values[0]
            orgV['COUNTY']=gDF.Admin2.values[0]
            orgV['GEONUM']=gDF.FIPS.values[0].astype('int') 
            orgV['PERDAY']=orgV['COUNT']
            lastVal=0
            for val in orgV['COUNT']: 
                orgV['PERDAY']=orgV['COUNT']
                orgV[orgV['PERDAY']==val]=val-lastVal
                lastVal=val
            break
        print(orgV)
#            orgV['DATE']=gDF.T[11:].index.values
...
#            orgV['COUNT']=gDF.T[11:].values.astype('int')
#            orgV['GEONUM']=orgV.at['GEONUM'] =int(gDF.FIPS.values[0])
#            orgV['COUNTY']=orgV.at['COUNTY'] = gDF.Admin2.values[0]
#            orgV['STATE']=orgV.at['STATE'] = gDF.Province_State.values[0]
#            orgV=orgV.drop(orgV.index[-3:]) 
#            newDF['STATE']=orgV.STATE.to_frame()
#            newDF['COUNTY' ]=orgV.COUNTY.to_frame()
#            newDF['PERDAY']=orgV.PERDAY.to_frame()
#            newDF['GEONUM']=orgV.GEONUM.to_frame()
#            newDF['DATE']=orgV.DATE.str.replace(' 00:00:00','')
#            newDF['PERDAY']=orgV.PERDAY.to_frame()
#            newDF=newDF.append(orgV,ignore_index=True)
#            newDF[newDF['PERDAY']<0]=0
#        print(newDF)  
